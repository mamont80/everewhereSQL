...
namespace ParserCore
{
    ...
    internal class ParserLexemClass
    {
        ...
        public void LexemParse(string s)
        {
            ...
            for (i = 0; i < s.Length; i++)
            {
                ...
                switch (c)
                {
                    ...
                    case '\'':
                    case '"':
                        if (status != LexType.Text && status != LexType.Command) { PushString(); cur.Append(c); TextQuote = c; status = LexType.Text; }
                        else if (status == LexType.Command)
                        {//меняем тип
                            if (PrevChar() != '.') MakeError();
        ...
        {
            res.Add(new LexExpr(){Lexem = text, LexemType = tp, ColNum = col, RowNum = row});
            status = LexType.None;
        }
        ...
        private void ReOpenString()
        {
            ...
            cur.Append(InsertSpecChars(res.Last().Lexem, res.Last().LexemType));
            TextQuote = ' ';
